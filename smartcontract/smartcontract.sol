...
contract SupplyChain {
    ...
    function newItem(string memory _text,string memory _det,string memory _prc,string memory _exp,string memory _ls, string memory _date) public returns (bool) {
        Product memory newItem = Product({creator: msg.sender, totalStates: 0,productName: _text ,productdetail: _det ,price: _prc ,productExp: _exp ,lotsize: _ls , productId: items, date: _date});
    ...
    function searchProduct(uint _productId) public returns (string memory) {
        ...
        output=concat(output, "<br>Product Details: ");
        output=concat(output, allProducts[_productId].productdetail);
        output=concat(output, "<br>Price / Unit: ");
        output=concat(output, allProducts[_productId].price);
        output=concat(output, "<br>Product Expiry: ");
        output=concat(output, allProducts[_productId].productExp);
        output=concat(output, "<br>Lot Size: ");
        output=concat(output, allProducts[_productId].lotsize);
